...
export default function FlashcardsClient({ cards, coreCount, supplementalCount }) {
  ...
  return <section>
    <div className="view-head"><div><h2>Course Flashcards</h2><p>Try to explain the concept before revealing the answer and exam focus.</p><small>{cards.length} total cards = {coreCount} core learning-section cards + {supplementalCount} supplemental cards.</small></div><div className="toolbar"><button className="btn" onClick={() => { setDeck([...deck].sort(() => Math.random() - .5)); setPos(0); setRevealed(false); }}>Shuffle</button></div></div>
