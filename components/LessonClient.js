...
export default function LessonClient({ lesson, previous, next, nextPreview }) {
  ...
  return <div className="lesson-page">
    ...
    <div className="lesson-intro"><h3>Lesson goals</h3><p>This lesson is self-contained. Read each section, work through examples, use the exam focus to understand how CompTIA may frame the concept, and answer the knowledge check before marking the section mastered.</p>{objectives.length > 0 && <ul className="lesson-objectives">{objectives.map((x) => <li key={x}>Explain or recognize <strong>{x}</strong> in a Security+ scenario.</li>)}</ul>}<p className="course-note">Below I provided the main <strong>vocabulary</strong> in case you want to do any further research.</p></div>
